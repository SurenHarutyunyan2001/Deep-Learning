...
# Загружаем модель с 4-битной квантованием
bnb_config = BitsAndBytesConfig(
    load_in_4bit = True,   # 4-битное квантование
    bnb_4bit_quant_type = "nf4",  # Тип квантования (NF4)
    bnb_4bit_use_double_quant = True  # Двойное квантование для точности
)
...
model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels = 2, quantization_config = bnb_config).to("cuda" if torch.cuda.is_available() else "cpu")  # Загружаем модель для классификации с двумя метками (положительный/отрицательный)
