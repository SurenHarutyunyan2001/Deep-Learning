...
# Генерация текста с подсказкой для Multi-token Prediction
def generate_text_with_prompt(prompt, model, tokenizer, max_len = 100, num_tokens = 5):
    input_tokens = tokenizer.encode(prompt, return_tensors = "tf")
    generated_sequence = input_tokens[0].numpy().tolist()

    for _ in range(max_len // num_tokens):  # Генерируем сразу несколько токенов
        ...
        next_tokens = tf.argmax(predictions[:, len(generated_sequence) - 1:len(generated_sequence) + num_tokens - 1, :], axis = -1).numpy()[0]
        generated_sequence.extend(next_tokens.tolist())

        if next_tokens[-1] == tokenizer.eos_token_id:  # Если токен <EOS>, прекращаем генерацию
            break
...
generated_text = generate_text_with_prompt(prompt, gpt1, tokenizer, max_len = 100, num_tokens = 5)
